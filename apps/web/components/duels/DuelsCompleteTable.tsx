...
import type { DuelDTO, MatchDTO, PlayerDTO } from "@repo/contracts";
						...
						<Table className="bg-card border">
							...
							<TableBody>
								...
								{playersInMonth.map((enemy) => {
									...
									return (
										<TableRow key={enemy.steamId} className="hover:bg-card">
											<TableCell>
												<Link
													href={`/profile/${enemy.steamId}`}
													className="flex flex-row gap-3 items-center w-fit cursor-pointer"
												>
