...
export const ClutchesTable = ({ clutches, teamA, teamB }: ClutchTableProps) => {
    ...
    const teamAPlayerIds = new Set(
        teamA.players.map(player => player.steamId)
    );

    const teamBPlayerIds = new Set(
        teamB.players.map(player => player.steamId)
    );

    clutches.forEach((clutch) => {
        if (teamAPlayerIds.has(clutch.clutcherSteamId)) {
            teamClutchMap.get(teamA.name)!.push(clutch)
        }
        else if (teamBPlayerIds.has(clutch.clutcherSteamId)) {
            teamClutchMap.get(teamB.name)!.push(clutch);
        }
    })
