...
export const ClutchProgress = ({ clutches }: ClutchProgressProps) => {
    ...
    const saveTotal = clutches.filter(c => c.clutcherSurvived === "true" && !c.hasWon).length;
    ...
    return (
        <div className="flex flex-col gap-1 px-4">
            ...
            <div className="flex flex-row items-start justify-between">
                <p className="text-gray-400">{winsTotal} {winsTotal === 1 ? "Clutch Ganho" : "Clutches Ganhos"}</p>
                <div className="flex flex-col items-end text-right">
                    <p className="text-gray-400">{lossTotal} {lossTotal === 1 ? "Clutch Perdido" : "Clutches Perdidos"}</p>
                    <p className="text-yellow-400">{saveTotal} {saveTotal === 1 ? "Save" : "Saves"}</p>
                </div>
            </div>
        </div>
    )
