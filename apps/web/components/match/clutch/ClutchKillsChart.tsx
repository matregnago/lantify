...
export const ClutchKillsChart = ({ clutches }: ClutchKillsChartProps) => {
	...
	const totalKills = clutches.reduce(
		(total, clutch) => total + clutch.clutcherKillCount,
		0,
	);
	const totalOpponents = clutches.reduce(
		(total, clutch) => total + clutch.opponentCount,
		0,
	);
	...
	return (
		<div className="flex items-start gap-0">
			...
			<div className="-ml-20 -mt-2 flex flex-col items-start">
				...
				<span>
					<ClutchKillToolTip kills={totalKills} total={totalOpponents} />
				</span>
			</div>
		</div>
	);
